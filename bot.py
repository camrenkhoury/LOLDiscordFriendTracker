...
async def on_command_error(ctx, error):
    # Makes “silent failures” visible while developing
...
async def playerinfo(ctx, riot_id: str):
    ...
    Example: !playerinfo KingCambro#NA1
    """
    if "#" not in riot_id:
        await ctx.send("❌ Use format: Name#TAG (example: KingCambro#NA1)")
        return
    ...
    # Ranked (from get_player_profile)
    ...
    # Top 5 Mastery
    # --------------------
    # If Riot blocks summoner-by-name (403), mastery cannot be fetched with current access.
    try:
        ...
        # Keep other features working; show a useful message instead of failing silently
        mastery_lines = "(unavailable — mastery requires summoner lookup that is currently forbidden)"
        print("MASTERY ERROR:", repr(e))

    # --------------------
    # Top 5 played champs in Solo/Duo (last 20 Solo games) W–L
    # --------------------
    try:
...
async def playerlist(ctx):
    ...
    if not players:
        await ctx.send("No players added yet. Use `!addplayer Name#TAG`.")
        return
...
async def updateseason(ctx):
    ...
    async with update_lock:
        ...
        for riot_id, p in data["players"].items():
            ...
            save_data(data)  # persist progress per player
...
async def dailyrecords(ctx):
    ...
    # ---------- totals + weighted avg kda ----------
    solo_w = solo_l = flex_w = flex_l = aram_w = aram_l = 0
    solo_k = solo_d = solo_a = 0
    flex_k = flex_d = flex_a = 0
    aram_k = aram_d = aram_a = 0
    ...
    for _, _, solo, flex, aram in rows:
        ...
        # Recover totals from KDA is not possible, so compute_wl_kda should be extended
        # For now, treat avg KDA as weighted by games using per-player KDA
        # (If you want true combined KDA, update compute_wl_kda to also return kills/deaths/assists)
        solo_kda_games = solo["games"]
        flex_kda_games = flex["games"]
        aram_kda_games = aram["games"]

    # Weighted-by-games average (simple, no extra API/data needed)
    def weighted_avg_kda(get_idx):
        total_g = 0
        total_kda = 0.0
        for _, _, solo, flex, aram in rows:
            x = [solo, flex, aram][get_idx]
            g = x["games"]
            total_g += g
            total_kda += x["kda"] * g
...
async def topduos(ctx):
    ...
    for r in results[:10]:
        lines.append(
            f"{r['duo']}  {r['wins']}-{r['games'] - r['wins']}  ({r['wr']:.1f}%) [{r['games']}g]"
        )
    lines.append("```")
